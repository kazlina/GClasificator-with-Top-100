				    ...
				    + " ("
				        + " select GPM.id as gpm_id, (postWordWeight.wordPercent) as veroyatnost_na_osnove_klyuchevyh_slov_lenty,"
				          ...
				          + " (profileWordWeight.wordPercent) as veroyatnost_na_osnove_klyuchevyh_slov_iz_profilya,"
				                ...
				                + " veroyatnost_na_osnove_procentnogo_sootnosheniya_tipov_kontenta"
				            + " from GroupDescr, GPM,"
				          + " ( select gpm.id as gpm_id, ifnull(weight_and_amount_all_words.wordWeight,0)/ifnull(weight_and_amount_all_words.amount,1)*100 as wordPercent"
				                + " from"
				                + " GPM"
				            + " left join"
				            + " ("
				                ...
				                + " from"
				                    + " ("
				                        + " select "
				                        + " GPM.id as gpm_id,"
				                            + " sum(post_word_weight.wordWeight) as wordWeight"
				                            + " from"
				                                + " GPM, "
				                            + " ( select post.gpm as gpm_id,"
                                    + " posts_word_weight.word_weight as wordWeight"
                              + " from"
                              + " post,"
                              + " ("
        ...
        + " ("
            + " select sumEachLink.gpm_id as gpm_id,"
                ...
                + " amountAllLink.amount as amount"
                    + " from"
                    + " ("
                          + " select GPM.id as gpm_id, sum(post_link_weight.linkWeight) as linkWeight"
                          + " from GPM"
                          + " ,"
                          + " ( "
